...
type Vista = 'qr' | 'docs' | 'historial' | 'perfil'
...
export default function PortalDocumentosPage() {
  ...
  async function buscarProveedor(e: React.FormEvent) {
    ...
    if (habData) {
      ...
      // Si está aprobado → mostrar QR primero
      if (data.estado === 'APROBADO') setVista('qr')
      else setVista('docs')
      ...
      <div className="min-h-screen bg-[#0f1117] flex items-center justify-center p-4">
        <div className="w-full max-w-sm">
          ...
          <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-8">
            ...
            <p className="text-zinc-500 text-sm mb-6">Ingresá tu CUIT para ver tu documentación y carnet de acceso.</p>
  ...
  const aprobado = proveedor.estado === 'APROBADO' && habilitacion
  ...
  return (
    <div className="min-h-screen bg-[#0f1117] text-white">
      ...
      <div className="max-w-lg mx-auto px-4 py-6">
        ...
        {/* ── VISTA QR (pantalla principal si está aprobado) ── */}
        {vista === 'qr' && aprobado && (
          <div className="flex flex-col items-center">
            ...
            <div className="flex items-center gap-2 mb-6">
              ...
              {habilitacion.fecha_venc && (
                <span className="text-zinc-500 text-xs">
                  hasta {new Date(habilitacion.fecha_venc).toLocaleDateString('es-AR')}
                </span>
              )}
            </div>
            ...
            {/* Instrucción */}
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl px-5 py-4 w-full text-center mb-6">
            ...
            <div className="flex gap-1 w-full bg-white/[0.03] border border-white/[0.06] rounded-xl p-1">
              {[
                ...
                <button key={tab.key} onClick={() => setVista(tab.key as Vista)}
        ...
        {/* ── VISTAS SECUNDARIAS (docs, historial, perfil) ── */}
        {vista !== 'qr' && (
          <>
            {/* Header con info + estado */}
                          ...
                          <div className="flex items-center justify-between mb-1">
                            <div className="flex items-center gap-2">
                              <span className={`text-xs px-2 py-0.5 rounded-full border ${
                                ...
                                ev.resultado === 'RECHAZADO' ? colorClass('red') : colorClass('yellow')
                              }`}>{ev.resultado.toLowerCase()}</span>
                              <span className="text-zinc-600 text-xs">{ev.tipo}</span>
                            </div>
                            <span className="text-zinc-600 text-xs">{new Date(ev.created_at).toLocaleDateString('es-AR')}</span>
                          </div>
                          {ev.observaciones && <p className="text-zinc-500 text-xs italic">"{ev.observaciones}"</p>}
                        </div>
                      ))}
                    </div>
                  </div>
                  ...
                  {editandoPerfil ? (
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        { label: 'Registrado', value: new Date(proveedor.created_at).toLocaleDateString('es-AR') },
                      ].map(({ label, value }) => (
                ...
                <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl p-5">
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-zinc-500 text-xs mt-0.5">Recibir email 7 días antes del vencimiento</p>
                    </div>
                    <button onClick={toggleNotif}
                      className={`relative rounded-full transition-colors`}
                      style={{ height: '22px', width: '40px', background: proveedor.notif_vencimientos ? '#2563eb' : 'rgba(255,255,255,0.1)' }}>
