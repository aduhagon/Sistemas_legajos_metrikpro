...
export default function ReportesClient({ stats, vencimientos, vencidos, porRubro, actividad, todosProveedores }: Props) {
  const [tab, setTab] = useState<'resumen' | 'vencimientos' | 'proveedores' | 'actividad'>('resumen')
            ...
            <div className="grid grid-cols-2 gap-4">
              {vencidos.length > 0 && (
                <div className="bg-red-500/5 border border-red-500/20 rounded-2xl p-5">
                  ...
                  <button onClick={() => setTab('vencimientos')} className="text-red-400 hover:text-red-300 text-xs transition-colors">
                    Ver detalle →
                  </button>
                </div>
              )}
              {vencimientos.length > 0 && (
                <div className="bg-yellow-500/5 border border-yellow-500/20 rounded-2xl p-5">
                  ...
                  <button onClick={() => setTab('vencimientos')} className="text-yellow-400 hover:text-yellow-300 text-xs transition-colors">
                    Ver detalle →
                  </button>
                </div>
              )}
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="text-zinc-400 text-sm">Mostrar próximos</span>
              {[7, 15, 30].map(d => (
                <button key={d} onClick={() => setFiltroDias(d)}
                  className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${
                    filtroDias === d ? 'bg-white/[0.08] text-white' : 'text-zinc-500 hover:text-zinc-300'
                  }`}>{d} días</button>
              ))}
            </div>
          ...
          {vencidos.length > 0 && (
            <div className="bg-red-500/5 border border-red-500/20 rounded-2xl overflow-hidden">
              <div className="px-5 py-3 border-b border-red-500/10">
                <span className="text-red-400 text-sm font-medium">🔴 Documentos vencidos ({vencidos.length})</span>
              </div>
                  ...
                  <div key={d.id} className="px-5 py-3 flex items-center justify-between">
                    <div>
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className="text-zinc-600 text-xs">{d.proveedores?.rubros?.nombre}</span>
                      </div>
                      <span className="text-zinc-500 text-xs">{d.documentos_requeridos?.nombre}</span>
                    </div>
                    <div className="text-right">
                      <p className="text-red-400 text-xs font-medium">
                        Venció el {new Date(d.fecha_venc).toLocaleDateString('es-AR')}
                      </p>
          ...
          {vencimientosFiltrados.length > 0 ? (
            <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl overflow-hidden">
              <div className="px-5 py-3 border-b border-white/[0.06]">
                <span className="text-yellow-400 text-sm font-medium">⚠️ Por vencer en {filtroDias} días ({vencimientosFiltrados.length})</span>
              </div>
              <div className="divide-y divide-white/[0.04]">
                    ...
                    <div key={d.id} className="px-5 py-3 flex items-center justify-between">
                      ...
                      <div className="text-right">
                        <p className="text-zinc-300 text-xs">
                          {new Date(d.fecha_venc).toLocaleDateString('es-AR')}
                        </p>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {['TODOS', 'PENDIENTE', 'EN_REVISION', 'APROBADO', 'RECHAZADO', 'SUSPENDIDO'].map(e => (
                <button key={e} onClick={() => setFiltroEstado(e)}
                  className={`px-3 py-1 rounded-lg text-xs font-medium transition-all ${
                    filtroEstado === e ? 'bg-white/[0.08] text-white' : 'text-zinc-500 hover:text-zinc-300'
                  }`}>
          ...
          <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl overflow-hidden">
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  <th className="text-left text-zinc-500 text-xs font-medium px-5 py-3">Empresa</th>
                  ...
                  <th className="text-left text-zinc-500 text-xs font-medium px-4 py-3">Docs</th>
                  <th className="text-left text-zinc-500 text-xs font-medium px-4 py-3">Estado</th>
                  <th className="text-left text-zinc-500 text-xs font-medium px-4 py-3">Registrado</th>
                    ...
                    <tr key={p.id} className={`border-b border-white/[0.04] hover:bg-white/[0.02] transition-colors ${i === proveedoresFiltrados.length - 1 ? 'border-0' : ''}`}>
                      ...
                      <td className="px-4 py-3">
                        <Link href={`/dashboard/legajos/${p.id}`}
                          className="text-blue-400 hover:text-blue-300 text-xs transition-colors">Ver →</Link>
                      </td>
                    </tr>
                  )
                })}
            ...
            {proveedoresFiltrados.length === 0 && (
              <div className="px-5 py-8 text-center">
                <p className="text-zinc-500 text-sm">No hay proveedores con ese estado</p>
              </div>
            )}
          </div>
        </div>
      ...
      {tab === 'actividad' && (
        <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl overflow-hidden">
          <div className="px-5 py-4 border-b border-white/[0.06]">
            ...
            <p className="text-zinc-500 text-xs mt-0.5">Últimas 20 acciones registradas en el sistema</p>
          </div>
          {actividad.length === 0 ? (
            <div className="px-5 py-8 text-center">
              <p className="text-zinc-500 text-sm">Sin actividad registrada todavía</p>
            </div>
          ) : (
            <div className="divide-y divide-white/[0.04]">
              {actividad.map((a: any) => (
                <div key={a.id} className="px-5 py-3 flex items-center gap-4">
                  <div className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 ${
                    ...
                    a.accion === 'RECHAZADO'         ? 'bg-red-500/10' :
                    a.accion === 'CRON_VENCIMIENTOS' ? 'bg-zinc-500/10' :
                    'bg-blue-500/10'
                  }`}>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
                      stroke={
                        a.accion === 'APROBADO'          ? '#22c55e' :
                        a.accion === 'RECHAZADO'         ? '#ef4444' :
                        a.accion === 'CRON_VENCIMIENTOS' ? '#71717a' :
                        '#60a5fa'
                      } strokeWidth="2">
                      {a.accion === 'APROBADO'
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-zinc-600 text-xs">{a.entidad}</span>
                    </div>
                    {a.datos_json && typeof a.datos_json === 'object' && (
                      <p className="text-zinc-600 text-xs truncate">
                        {JSON.stringify(a.datos_json).slice(0, 80)}
                      </p>
                    )}
                  </div>
