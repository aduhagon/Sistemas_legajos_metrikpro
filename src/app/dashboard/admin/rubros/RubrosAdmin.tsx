...
export default function RubrosAdmin({ rubros: rubrosIniciales }: { rubros: Rubro[] }) {
  ...
  const [nuevoDoc, setNuevoDoc] = useState<string | null>(null) // rubro_id
  ...
  // ── Rubro: editar nombre/descripcion ──
  ...
  // ── Documento: toggle activo ──
  async function toggleDocActivo(rubroId: string, doc: DocReq) {
    setLoading(true)
  ...
  function abrirEditDoc(doc: DocReq) {
    setDocForm({
      codigo: doc.codigo, nombre: doc.nombre, tipo_vigencia: doc.tipo_vigencia,
      obligatorio: doc.obligatorio,
      aplica_persona_fisica: doc.aplica_persona_fisica,
  ...
  async function guardarDoc(rubroId: string, docId: string) {
    setLoading(true)
    ...
    setRubros(r => r.map(r2 => r2.id === rubroId
      ? { ...r2, documentos_requeridos: r2.documentos_requeridos.map(d =>
          d.id === docId ? { ...d, ...docForm } : d) }
      : r2))
    setEditandoDoc(null)
    setLoading(false)
  }

  // ── Documento: agregar nuevo ──
  function abrirNuevoDoc(rubroId: string) {
    setDocForm(emptyDocForm)
    setNuevoDoc(rubroId)
    setEditandoDoc(null)
  }

  async function crearDoc(rubroId: string) {
    setLoading(true)
  ...
  function DocFormFields({ onSave, onCancel, saving }: { onSave: () => void; onCancel: () => void; saving: boolean }) {
    return (
      <div className="bg-white/[0.03] border border-white/[0.08] rounded-xl p-4 mt-2 space-y-3">
        ...
        <div className="flex gap-2 pt-1">
          <button onClick={onSave} disabled={saving || !docForm.codigo || !docForm.nombre}
              ...
              {editandoRubro === rubro.id ? (
                <div className="flex items-center gap-2">
                  <input value={rubroForm.nombre} onChange={e => setRubroForm(f => ({ ...f, nombre: e.target.value }))}
                    className="bg-white/[0.05] border border-white/[0.1] rounded-lg px-3 py-1.5 text-white text-sm focus:outline-none focus:border-blue-500/60 w-48"/>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-zinc-600 text-xs">
                    {rubro.documentos_requeridos.filter(d => d.activo).length} docs activos
                  </span>
                </div>
              )}
            </div>
                    ...
                    {editandoDoc === doc.id ? (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-2 shrink-0">
                          ...
                          <button onClick={() => toggleDocActivo(rubro.id, doc)} disabled={loading}
              ...
              <div className="px-5 py-3 border-t border-white/[0.04]">
                {nuevoDoc === rubro.id ? (
                  <DocFormFields
                    ...
                    onCancel={() => setNuevoDoc(null)}
                    saving={loading}
                  />
                ) : (
                  <button onClick={() => abrirNuevoDoc(rubro.id)}
