...
export default function AccionesDocumento({
  ...
  async function aprobarDoc() {
    setLoading(true)
    const { data: { user } } = await supabase.auth.getUser()
    await supabase.rpc('aprobar_documento', {
      p_doc_id: docId,
  ...
  async function rechazarDoc() {
    setLoading(true)
    const { data: { user } } = await supabase.auth.getUser()
    await supabase.rpc('rechazar_documento', {
      p_doc_id: docId,
      p_evaluador_id: user?.id,
      p_observaciones: obs,
    })
          ...
          {editandoFecha ? (
            <>
              ...
              <button onClick={guardarFecha} disabled={guardandoFecha}
                className="bg-blue-500/10 hover:bg-blue-500/20 text-blue-400 border border-blue-500/20 text-xs px-3 py-1 rounded-lg transition-all disabled:opacity-40">
      ...
      {estado === 'CARGADO' && (
        <>
          {showObs ? (
            ...
            <div className="flex items-center gap-2">
              <button onClick={aprobarDoc} disabled={loading}
