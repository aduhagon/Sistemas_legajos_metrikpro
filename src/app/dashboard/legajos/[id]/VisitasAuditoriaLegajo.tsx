...
export default function VisitasAuditoriaLegajo({
  ...
  async function supervisar(visita: Visita, estado: 'APROBADA' | 'RECHAZADA') {
    if (estado === 'RECHAZADA' && !obsRechazo.trim()) { setMsg('Ingresá una observación para rechazar'); return }
    setLoading(true)
    const { data } = await supabase.rpc('supervisar_visita_auditoria', {
      p_visita_id: visita.id, p_estado: estado,
      p_obs: estado === 'RECHAZADA' ? obsRechazo : null,
    })
    if (data?.ok) {
      setVisitas(prev => prev.map(v =>
        v.id === visita.id ? { ...v, estado_supervision: estado, supervision_obs: obsRechazo || null } : v
      ))
      setDetalle(null); setObsRechazo('')
      setMsg(`Visita ${estado === 'APROBADA' ? 'aprobada' : 'rechazada'}`)
  ...
  if (visitas.length === 0) {
    return (
      <div className="bg-white/[0.03] border border-white/[0.08] rounded-2xl px-6 py-10 text-center">
        <p className="text-zinc-500 text-sm">Este proveedor no tiene visitas de auditoría registradas</p>
      </div>
    )
  }
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-end sm:items-center justify-center p-4">
          <div className="bg-[#13151f] border border-white/[0.1] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/[0.08]">
              <div>
                <p className="font-medium">Detalle de visita</p>
            ...
            <div className="px-6 py-4 space-y-4">
              ...
              <div className={`rounded-xl border px-4 py-3 ${SUPERVISION_COLOR[detalle.estado_supervision]}`}>
                ...
                {detalle.supervision_obs && <p className="text-xs mt-1 opacity-80">{detalle.supervision_obs}</p>}
              </div>
              ...
              {puedeSuperviar && detalle.estado_supervision === 'PENDIENTE' && (
                <div className="space-y-3 pt-2 border-t border-white/[0.08]">
                  ...
                  <div className="space-y-2">
                    <textarea value={obsRechazo} onChange={e => setObsRechazo(e.target.value)} rows={2}
